...
async def build_knowledge_base(args):
    ...
    """
    builder = KnowledgeBuilder(chunk_size=args.chunk_size, chunk_overlap=args.chunk_overlap)
    ...
    try:
        ...
        if args.file:
            ...
            result = await builder.build_from_file(
                file_path=args.file, metadata=metadata, extract_faq=not args.no_faq
            )
        elif args.text:
            logger.info("Building knowledge base from provided text")
            result = await builder.build_from_text(
                text=args.text, metadata=metadata, extract_faq=not args.no_faq
            )
        else:
        ...
        # 打印结果
        logger.info(
            f"Stored {result['stored_chunks']}/{result['total_chunks']} chunks to knowledge base"
        )
        if not args.no_faq:
...
async def query_knowledge_base(args):
    ...
    """
    retriever = KnowledgeRetriever(max_search_results=args.max_results)
...
def print_usage_guide():
    ...
    print("\n使用自定义分块大小:")
    print(
        f"  python -m app.main build --file <文件路径> --chunk-size {DEFAULT_CHUNK_SIZE*2} --chunk-overlap {DEFAULT_CHUNK_OVERLAP*2}"
    )
    print("\n从文本构建知识库:")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Milvus MCP Client for knowledge base management and querying"
    )
    ...
    build_parser.add_argument(
        "--chunk-size", type=int, default=DEFAULT_CHUNK_SIZE, help="Size of text chunks"
    )
    build_parser.add_argument(
        "--chunk-overlap",
        type=int,
