...
class MCPClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully connected to MCP server. Available tools: {list(self.tools.keys())}"
            )
    ...
    async def store_knowledge(
        ...
        try:
            ...
            logger.info("Successfully stored knowledge content")
            return response
        except Exception as e:
                ...
                for part in response.content:
                    if hasattr(part, "text") and part.text:
                        try:
                            ...
                            # 如果不是有效的JSON，按原样使用文本
                            logger.warning(
                                f"Could not parse JSON from searchKnowledge response: {part.text}"
                            )
    ...
    async def store_faq(
        ...
        try:
            ...
            logger.info("Successfully stored FAQ content")
            return response
        except Exception as e:
                ...
                for part in response.content:
                    if hasattr(part, "text") and part.text:
                        try:
                            ...
                            # 如果不是有效的JSON，按原样使用文本
                            logger.warning(
                                f"Could not parse JSON from searchFAQ response: {part.text}"
                            )
        ...
        if self._connected:
            try:
                ...
                try:
                    ...
                    if "generator didn't stop after athrow()" in str(
                        e
